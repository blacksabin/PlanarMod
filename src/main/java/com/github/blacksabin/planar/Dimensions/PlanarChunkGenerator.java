...
public class PlanarChunkGenerator extends ChunkGenerator {
    public static final Codec<PlanarChunkGenerator> CODEC = RecordCodecBuilder.create((instance) ->
            method_41042(instance).and(
                            RegistryOps.createRegistryCodec(Registry.BIOME_KEY).forGetter((generator) -> generator.biomeRegistry)
                    )
                    .apply(instance, instance.stable(PlanarChunkGenerator::new))
    );

    private final Registry<Biome> biomeRegistry;

    public PlanarChunkGenerator(Registry<StructureSet> registry, Registry<Biome> biomeRegistry) {
